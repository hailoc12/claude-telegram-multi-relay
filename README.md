...
Bạn chạy Claude Code trên một máy (Mac, headless server, hoặc mini luôn-on). Vấn
đề: Claude là CLI trong terminal, bạn phải ngồi trước máy mới dùng được.

`claude-telegram-multi-relay` biến Claude Code thành **một đội nhân viên số mà
bạn trò chuyện qua Telegram trên điện thoại**:

- **Mỗi Claude session bằng một topic** trong Telegram group (kiểu Slack channel).
- **Nhiều session chạy song song**, mỗi cái làm một project riêng.
- **Một bot phục vụ nhiều group**: multi-user, mỗi đội hoặc khách hàng một không
  gian riêng.
- **Hai chiều thời gian thực**: bạn gõ, Claude làm, Claude trả lời về topic.
- **Tự lên lịch** mỗi sáng, mỗi 15 phút, mỗi thứ Sáu, sống qua reboot và sleep.
- **Tự chữa** (auto-restart), kèm broadcast, audit fleet, monitor.

Tất cả được đóng gói thành một **Claude Code Skill**. Bạn nói *"tạo 3 session
...
Relay làm hai việc. Hướng vào: tin nhắn Telegram được đẩy vào Claude đang chạy.
Hướng ra: output của Claude được gửi về Telegram.

Hai chiều chạy cùng lúc, nên bạn và Claude trò chuyện qua Telegram như chat với
người thật, chỉ là "người" đó đọc và sửa file, chạy lệnh, deploy ngay trên máy
bạn.

Vì sao Telegram? Có sẵn mọi nền tảng, Bot API miễn phí và mạnh, có **Forum
Topics** (tách session như channel), HTML mode định dạng đẹp, không rate-limit
khắt khe.
...
Relay đơn giản chỉ nối một Claude với một chat. Repo này thêm ba trục:

| Trục | Nghĩa |
|---|---|
| **Multi-session** | N Claude Code chạy song song, mỗi cái có project, context, lịch sử riêng |
| **Multi-topic** | Mỗi session bằng một Telegram forum topic. Mở group ra thấy danh sách đội |
| **Multi-user / multi-group** | **Một bot phục vụ nhiều group.** Session thuộc group nào reply về group đó |

Bidirectional thực sự không phải kiểu "gửi task, chờ email". Mà là hội thoại thời
gian thực:
...
Mỗi reply là một message Telegram, nên reply, forward, pin, search lại lúc nào
cũng được. Toàn bộ tương tác với đội AI nằm gọn trong Telegram, ứng dụng bạn đã
có sẵn.
...
Khoảng trống repo này lấp: *N Claude ứng với N topic ứng với N group*, kèm lập lịch
persistent và quản lý fleet. Chưa project nào ở trên làm phần này.

Khi nào dùng gì:
- Chỉ chat một Claude: dùng thẳng [`ccc`](https://github.com/kidandcat/ccc).
- Muốn nhiều kênh (email, discord): JessyTsui/Claude-Code-Remote.
- Muốn SaaS, không host: Claudegram.
- **Muốn chạy đội AI nhiều session, tự lên lịch, self-heal, điều khiển từ điện
  thoại: repo này.**

> 📖 Phân tích đầy đủ: [`docs/02-comparison.md`](./docs/02-comparison.md)

Trade-off thành thật: macOS-first (scheduling dùng `launchd`); phụ thuộc `ccc`
(không reimplement relay, cố ý, để kế thừa engine ổn định); lý tưởng nhất cho máy
always-on hoặc headless server.

---

## Vị trí trong AI Native Company

Repo này là **tầng giao tiếp và vận hành của con người** (human communication &
operations layer) cho mô hình [**AI Native Company**](https://github.com/hailoc12/ai_native_company),
nơi một người vận hành với năng lực 15 đến 20 người nhờ đội AI Worker.
...
(marketing, sales, finance, research...). Repo này cho con người **cơ chế** để:

- **giao việc** cho từng Worker (và Worker trả lời lại), qua Telegram topic,
- **xem cả đội** đang làm gì (list, status, monitor),
...
Nói cách khác: nếu `ai_native_company` là bộ não và sơ đồ tổ chức (định nghĩa
Worker), thì repo này là hệ thống liên lạc nội bộ cộng ca trực, nơi con người
...
**Đội nghiên cứu song song.** Năm Claude, mỗi con một đối thủ, broadcast một lệnh:
```bash
...
**Đội tự chữa.** Session chết thì tự restart, quá giới hạn thì notify:
```bash
...
**Tóm lại:** `ccc` là *engine relay một session*. Repo này là *bộ chỉ huy điều
phối đội nhiều session, kèm lịch trình và self-heal*. Hai bên kết nối qua config
của ccc (`~/.config/ccc/config.json`), repo này đọc và ghi config đó như nguồn
dữ liệu chính.
